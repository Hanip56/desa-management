...
import { getAllSkDomisiliLembaga } from "@/fetcher/sk-domisili-lembaga-fetcher";
...
export const SkDomisiliLembagaClient = () => {
  const [
    page,
    handleNext,
    handlePrevious,
  ...
  const query = useQuery({
    queryKey: ["sk-domisili-lembagas", { page, search, status, updatedAt }],
    queryFn: () =>
      getAllSkDomisiliLembaga({
        page,
        limit,
        search,
        status,
      ...
      <CardHeader className="flex flex-col md:flex-row items-center md:justify-between gap-y-2">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Button asChild className="w-full md:w-fit">
            <Link href="sk-domisili-lembaga/formulir">
