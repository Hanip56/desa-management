...
import { SkBelumMenikahType } from "./columns";
import { useRouter } from "next/navigation";
import { useConfirm } from "@/hooks/use-confirm";
import Link from "next/link";
import { useSession } from "next-auth/react";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { deleteSkBelumMenikah } from "@/fetcher/sk-belum-menikah-fetcher";
import { toast } from "sonner";

type CellActionProps = {
  data: SkBelumMenikahType;
};
...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteSkBelumMenikah,
    onSuccess: (data) => {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          {session?.user.role === "USER" && data.status === "DIPROSES" && (
            <>
              <DropdownMenuItem
                onClick={() => router.push(`sk-belum-menikah/${data.id}`)}
              >
