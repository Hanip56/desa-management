...
import { deleteSkPenghasilanOrangTua } from "@/fetcher/sk-penghasilan-orang-tua-fetcher";
...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteSkPenghasilanOrangTua,
    onSuccess: (data) => {
        ...
        <DropdownMenuContent align="end">
          {(session?.user.role === "ADMIN" || data.status !== "DIPROSES") && (
            <DropdownMenuItem asChild>
              <Link
                href={`sk-penghasilan-orang-tua/${data.id}`}
          ...
          {session?.user.role === "USER" && data.status === "DIPROSES" && (
            <>
              <DropdownMenuItem
                onClick={() =>
                  router.push(`sk-penghasilan-orang-tua/${data.id}`)
                }
              >
