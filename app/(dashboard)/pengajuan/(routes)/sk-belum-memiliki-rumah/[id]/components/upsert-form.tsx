...
import { skBelumMemilikiRumahSchema } from "@/schemas/sk-belum-memiliki-rumah";
...
import {
  createSkBelumMemilikiRumah,
  updateSkBelumMemilikiRumah,
} from "@/fetcher/sk-belum-memiliki-rumah-fetcher";
...
type Props = {
  initialData?: SkBelumMemilikiRumahWithUser | null;
};
...
const UpsertForm = ({ initialData }: Props) => {
  ...
  const form = useForm<z.infer<typeof skBelumMemilikiRumahSchema>>({
    resolver: zodResolver(skBelumMemilikiRumahSchema),
    defaultValues: {
  ...
  const createMutation = useMutation({
    mutationFn: createSkBelumMemilikiRumah,
    onSuccess: (data) => {
      toast("Data berhasil diajukan.", {
        className: "text-emerald-600 font-semibold",
      });
      form.reset();
      router.push("/pengajuan/sk-belum-memiliki-rumah");
  ...
  const updateMutation = useMutation({
    mutationFn: updateSkBelumMemilikiRumah,
    onSuccess: (data) => {
  ...
  const onSubmit = async (
    values: z.infer<typeof skBelumMemilikiRumahSchema>
  ) => {
    if (initialData) {
