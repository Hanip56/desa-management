...
export const columns = (toggleSortDate: () => void) => {
  const columns: ColumnDef<ColumnsType>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <p className="text-center">{row.original.noKk}</p>,
    },
    {
      accessorKey: "nik",
      header: () => (
        <div className="flex items-center justify-center text-center">NIK</div>
      ),
      cell: ({ row }) => <p className="text-center">{row.original.nik}</p>,
    },
    {
      accessorKey: "status",
      header: () => (
