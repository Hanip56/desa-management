...
import { deleteSkTidakMemilikiPekerjaan } from "@/fetcher/sk-tidak-memiliki-pekerjaan-fetcher";
...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteSkTidakMemilikiPekerjaan,
    onSuccess: (data) => {
        ...
        <DropdownMenuContent align="end">
          {(session?.user.role === "ADMIN" || data.status !== "DIPROSES") && (
            <DropdownMenuItem asChild>
              <Link
                href={`sk-tidak-memiliki-pekerjaan/${data.id}`}
          ...
          {session?.user.role === "USER" && data.status === "DIPROSES" && (
            <>
              <DropdownMenuItem
                onClick={() =>
                  router.push(`sk-tidak-memiliki-pekerjaan/${data.id}`)
                }
              >
