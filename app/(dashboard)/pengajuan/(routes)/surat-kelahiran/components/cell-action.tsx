...
import { SuratKelahiranType } from "./columns";
import { useRouter } from "next/navigation";
import { useConfirm } from "@/hooks/use-confirm";
import Link from "next/link";
import { useSession } from "next-auth/react";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { deleteSuratKelahiran } from "@/fetcher/surat-kelahiran-fetcher";
...
const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: deleteSuratKelahiran,
    onSuccess: (data) => {
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          {session?.user.role === "USER" && data.status === "DIPROSES" && (
            <>
              <DropdownMenuItem
                onClick={() => router.push(`surat-kelahiran/${data.id}`)}
              >
