import Header from "@/app/(dashboard)/components/header";
import ClientComp from "./components/client-comp";
import prisma from "@/db/prisma";
import { getSignedUrl } from "@/lib/server-utils";
import { auth } from "@/auth";

const DynamicPage = async ({ params }: { params: { id: string } }) => {
  const session = await auth();
  let initialData: any;
  let skDomisiliImigrasi = await prisma.skDomisiliImigrasi.findUnique({
    where: { id: params.id },
    include: {
      User: {
        select: {
          id: true,
          username: true,
          nomorWa: true,
          role: true,
          ktpUrl: true,
          kkUrl: true,
          createdAt: true,
          updatedAt: true,
        },
      },
    },
  });

  if (skDomisiliImigrasi) {
    initialData = {
      ...skDomisiliImigrasi,
      User: undefined,
      user: {
        ...skDomisiliImigrasi.User,
        ktpUrl:
          skDomisiliImigrasi.User.ktpUrl && session?.user.role !== "USER"
            ? getSignedUrl(skDomisiliImigrasi.User.ktpUrl)
            : undefined,
        kkUrl:
          skDomisiliImigrasi.User.kkUrl && session?.user.role !== "USER"
            ? getSignedUrl(skDomisiliImigrasi.User.kkUrl)
            : undefined,
      },
    };
  }

  return (
    <main className="max-w-screen-lg mx-auto">
      <Header
        title={`Formulir SK Domisili Imigrasi`}
        className="text-center"
        withBreadcrumb
        breadcrumbClassName="flex justify-center"
      />

      <ClientComp initialData={initialData} />
    </main>
  );
};

export default DynamicPage;
